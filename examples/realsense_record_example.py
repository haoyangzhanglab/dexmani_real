...
(open3d non-blocking) -> performance summary -> config variant comparison.
...
def _build_hud_lines(
    ...
    if state.show:
        lines.append(
            f"PCD ON  n={stats['points']:.0f}  vox={state.voxel_size}  "
    ...
    lines.append(
        f"cmap={state.colormap}  depth=[{_DEFAULT_PCD_MIN_DEPTH:.2f},{_DEFAULT_PCD_MAX_DEPTH:.2f}]"
    )
    return lines
...
def _handle_keyboard(
    key: int, state: PointCloudDisplayState, viewer: NonBlockingPCDViewer
) -> bool:
...
def _toggle_ws(state: PointCloudDisplayState) -> None:
    state.toggle_workspace()
    print(f"  Workspace: {'ON' if state.workspace_on else 'OFF'}")


def _cycle_sampling(state: PointCloudDisplayState) -> None:
...
def _print_state(state: PointCloudDisplayState) -> None:
    ...
    print(
        f"  npoints={pcd.npoints}  sampling={pcd.sampling}  voxel={pcd.voxel_size}  "
...
def _run_rgbd_test(camera: RealSense, test_cfg: RealSenseDiagnosticConfig) -> dict:
    ...
    print(
        "   q/Esc=quit  p=pcd  v=voxel  w=workspace  s=sampling  r=reset  c=config  d=colormap"
    )
    ...
    print(
        f"  Start: npoints={pcd_config.npoints}  sampling={pcd_config.sampling}  "
        ...
        if state.show:
            ...
            try:
                ...
                pcd_array = rgbd_to_pointcloud(
                    depth=frame.depth, K=frame.K, rgb=frame.rgb, config=pcd_config
                )
...
def _build_variants() -> list[tuple[str, PointCloudConfig]]:
    ...
    return [
        (
            "random 1024 (default)",
            PointCloudConfig(npoints=1024, sampling="random", **base),
        ),  # type: ignore[arg-type]
        (
            "voxel 5mm + fps 1024",
            PointCloudConfig(npoints=1024, sampling="fps", voxel_size=0.005, **base),
        ),  # type: ignore[arg-type]
        (
            "voxel 10mm + fps 1024",
            PointCloudConfig(npoints=1024, sampling="fps", voxel_size=0.01, **base),
        ),  # type: ignore[arg-type]
        (
            "no sampling (full)",
            PointCloudConfig(sampling="none", **base),
        ),  # type: ignore[arg-type]
        (
            "fps 2048 (no voxel)",
            PointCloudConfig(npoints=2048, sampling="fps", **base),
        ),  # type: ignore[arg-type]
        (
        ...
        (
            "with workspace crop",
            PointCloudConfig(
                npoints=1024, sampling="random", workspace=_DEFAULT_WORKSPACE, **base
            ),
        ),  # type: ignore[arg-type]
        (
            "dense random 4096",
            PointCloudConfig(npoints=4096, sampling="random", **base),
        ),  # type: ignore[arg-type]
    ]
