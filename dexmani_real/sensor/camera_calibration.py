...
from dexmani_real.config.runtime import (
    ArmLoopConfig,
...
def _run_calibration(
    ...
    preview_detector = cv2.aruco.ArucoDetector(
        cv2.aruco.getPredefinedDictionary(_ARUCO_DICT),
    ...
    def _event_solve() -> None:
        ...
        if n < calib_cfg.min_samples:
            print(
                f"  need at least {calib_cfg.min_samples} samples, have {n} — keep collecting"
            )
            return
        print(f"\n  computing hand-eye calibration ({n} samples, 5 methods)...")
        try:
            T_candidate, method_best, errors_mm, errors_deg, method_table = (
        ...
        T_world_base[:3, 3] = np.asarray(
            planner.kin.base_pose_world.p, dtype=np.float64
        )
        ...
        print(
            f"    position mean={errors_mm.mean():.1f}mm max={errors_mm.max():.1f}mm std={std_mm:.1f}mm"
        )
        print(
            f"    rotation mean={errors_deg.mean():.2f}° max={errors_deg.max():.2f}° std={std_deg:.2f}°"
        )
        ...
        if pos_ok and rot_ok:
            ...
            print(
                f"  ACCEPTED ({method_best}, pos std={std_mm:.1f}mm, rot std={std_deg:.2f}°)"
            )
        else:
            reasons = []
            if not pos_ok:
                reasons.append(
                    f"pos std={std_mm:.1f}mm > {calib_cfg.max_consistency_std_mm:.1f}mm"
                )
            if not rot_ok:
                reasons.append(
                    f"rot std={std_deg:.2f}° > {calib_cfg.max_consistency_rot_std_deg:.1f}°"
                )
            print(
                f"  REJECTED (quality gate: {'; '.join(reasons)}) — increase rotation variety and retry"
            )
    ...
    try:
        while shared.is_running.value:
            ...
            while event is not None:
                if event == "space":
                    ...
                    if removed is None:
                        ...
                        idx, r = removed
                        print(
                            f"  removed worst frame #{idx + 1} (residual {r:.1f}mm), "
            ...
            if feedback_issue is not None:
                if quit_requested:
                    _set_fault(
                        shared, f"cannot publish measured quit hold: {feedback_issue}"
                    )
                    return 1
                continue
            ...
            workspace_margin_m = float(
                runtime.keyboard_teleop.workspace_command_margin_m
            )
            ...
            if frame % calib_cfg.status_interval_frames == 0:
                ...
                print(
                    f"[f={frame}] samples={len(samples)} "
                    f"eef={np.round(measured_pose.p, 3)}m "
                    f"target={np.round(target_pos, 3)}m",
                    flush=True,
                )
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        f"  ArUco: {_ARUCO_DICT_NAME} ID={aruco_cfg.target_id} "
        f"size={aruco_cfg.marker_size_m * 1000:.1f}mm"
    )
