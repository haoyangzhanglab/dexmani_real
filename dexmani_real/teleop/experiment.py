"""VR teleoperation experiment setup, supervision, and shutdown.

Spawn-only capability model: VR, teleop, arm, measured hand (unless explicitly
disabled), and camera plus
RecorderIO only when recording is enabled.
    ...
    *,
    ...
    if arm_result is None:
        ...
        if int(arm["error_code"][0]) != 0:
            arm_issue = f"arm reported controller error {int(arm['error_code'][0])}"
    ...
    *,
    ...
    if recording_enabled:
        ...
        recorder_config = RecorderIOConfig(
            data_dir=str(repo_root / policy_config.episodes_dir),
...
def _readiness_checks(
    ...
    *,
    ...
    checks = [
        ...
        ("teleop", shared.policy_ready, float(timeouts["policy"])),
    ]
    if recording_enabled:
        checks.extend(
            (
