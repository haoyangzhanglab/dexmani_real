...
def _quality_from_payload(payload: Any) -> VRTransformQuality:
    ...
    if grade not in {"excellent", "good", "poor"}:
        raise ValueError("VR calibration quality.grade must be excellent, good, or poor")
    try:
    ...
    expected_grade = "excellent" if std_deg < 2.0 else "good" if std_deg < 5.0 else "poor"
...
def load_vr_transform(path: str | Path, *, reject_poor: bool = True) -> VRTransformCalibration:
    ...
    transform = validate_rotation_matrix(payload.get("T_vr_to_robot"), name="T_vr_to_robot")
    ...
    if not np.allclose(transform, expected_transform, atol=_ROTATION_ATOL, rtol=0.0):
        raise ValueError("T_vr_to_robot disagrees with theta_deg and the declared convention")
