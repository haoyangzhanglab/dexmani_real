"""Pure joint-path helpers and typed collision-checked home-path results.
...
def _legacy_candidate(candidate: HomePathCandidate) -> dict[str, Any]:
    """Represent typed diagnostics for callers of the compatibility API."""
    result: dict[str, Any] = {
        "name": candidate.name,
        "safe": candidate.safe,
    ...
    optional_values = {
        ...
        "collision_waypoint_index": candidate.collision_waypoint_index,
        "workspace_segment_index": candidate.workspace_segment_index,
        "table_waypoint_index": candidate.table_waypoint_index,
        "table_check_source": candidate.table_check_source,
        "table_raw_measurement_m": candidate.table_raw_measurement_m,
        "clearance_m": candidate.clearance_m,
        "minimum_table_clearance_m": candidate.minimum_table_clearance_m,
    }
    result.update(
        {
            key: value
    ...
    if candidate.collision_pairs:
        result["collision"] = {
            "collision_pairs": [
                {"link1": pair.link1, "link2": pair.link2, "type": pair.kind}
                for pair in candidate.collision_pairs
            ]
        }
    if candidate.table_soft_escape:
        result["table_soft_escape"] = True
    return result
...
def plan_joint_home_path(
    ...
    *,
    ...
    """Compatibility wrapper preserving the legacy tri-state array result."""
    result = compute_joint_home_path(
        qpos,
        home_qpos,
        planner,
        table_z_surface_m=table_z_surface_m,
        hand_safety_margin_m=hand_safety_margin_m,
        use_canonical_target=use_canonical_target,
    )
    if report is not None:
        report.clear()
        report["status"] = result.status.value
        report["candidates"] = [
            _legacy_candidate(candidate) for candidate in result.candidates
        ]
        if result.selected_candidate is not None:
            report["selected_candidate"] = result.selected_candidate
    if result.status is HomePathStatus.ALREADY_HOME:
        return None
    return result.waypoints.copy()
...
def plan_band_alignment_path(
    ...
    *,
    ...
    """Compatibility wrapper preserving the legacy tri-state array result."""
    result = compute_band_alignment_path(
        wrapped_home,
        canonical_home,
        planner,
        table_z_surface_m=table_z_surface_m,
