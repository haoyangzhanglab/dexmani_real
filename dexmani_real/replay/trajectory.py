...
from dexmani_real.dataset.provenance import read_provenance_workflow, TELEOP_WORKFLOW
...
def load_trajectory(episode_path):
    ...
    with EpisodeReader(path) as reader:
        ...
        arm, hand, aq, hq = [h5[k][:] for k in ("action_arm_joint_target", "action_hand_joint_target", "arm_qpos", "hand_qpos")]
        ...
        return TrajectoryData(path, len(arm), reader.timing.rate_hz,
            str(h5["meta"].attrs.get("task_label", "")), arm, hand, aq, hq,
...
def verify_replay_preflight(trajectory, runtime):
    ...
    for values, lower, upper in (
        ...
        if not np.isfinite(values).all() or np.any(values < np.asarray(lower)-1e-9) or np.any(values > np.asarray(upper)+1e-9):
