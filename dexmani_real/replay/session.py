...
from dexmani_real.replay.replayer import replay_targets, ReplayStatus, ReplayOutcome
        ...
        if outcome.successful:
            ...
            while time.monotonic() < deadline and check_processes(shared, started):
                ...
                if OperatorCommand.HOME in signals:
                    ok = home_policy_robot(shared, runtime, build_policy_home_planner(runtime),
                        abort_requested=lambda: bool(shared.estop_request.value) or not keyboard.healthy)
                    if not ok:
                        outcome = ReplayOutcome(ReplayStatus.REJECTED, outcome.replay_data, "return_home failed")
                    break
    finally:
        keyboard.quiesce()
        report = shutdown_processes_verified(shared, started,
            graceful_timeout_s=runtime.safety.shutdown_timeout_s, disarm_if_clean=outcome.successful)
        keyboard.stop()
    if shared.error_state.value or shared.estop_request.value or not report.shared_closed or any(x.exitcode != 0 or x.escalation != "graceful" for x in report.exits):
        outcome = ReplayOutcome(ReplayStatus.FAULT, outcome.replay_data, "hardware/shutdown failure")
    evaluate_replay(trajectory, outcome.replay_data, evaluate_consistency=config.evaluate_consistency, output_dir=config.output_dir)
    return outcome
