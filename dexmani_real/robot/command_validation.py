...
def worker_validate_arm(
    ...
    *,
    ...
    """Validate shape, finiteness, generation, sequence, and expiry at the worker.

    Mirrors ``worker_validate_hand``: the arm command now carries the same
    ``run_generation`` / ``valid_until`` identity, so a STOP/FAULT generation
    bump rejects an in-flight endpoint before the SDK boundary.
    """
    well_formed = (
