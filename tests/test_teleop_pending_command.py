"""Offline regressions for pending-command cancellation and recording prefixes."""
...
from dexmani_real.teleop.control_loop.grid import drop_pending_command


class PendingCommandCancellationTest(unittest.TestCase):
    def test_pause_discards_uncommitted_intent(self):
        controller = SimpleNamespace(pending_publish=SimpleNamespace(
            candidate=SimpleNamespace(action_id=813)))
        drop_pending_command(controller, "pause_boundary")
        self.assertIsNone(controller.pending_publish)

    def test_repeated_cancellation_without_pending_intent_is_harmless(self):
