...
def test_arm_joint_limit_clip():
    robot = _stub_robot()
    robot.arm.qpos_min_soft = np.full(7, -2.0, dtype=np.float64)
    robot.arm.qpos_max_soft = np.full(7, 2.0, dtype=np.float64)
    ...
    assert ok
    assert action.arm_qpos_cmd[0] == 2.0, "upper bound not clipped"
    assert action.arm_qpos_cmd[1] == -2.0, "lower bound not clipped"
