...
`examples/visualize_episode.py` 对 raw v25 默认启用点云。持久化边界由
...
运行时只读取 raw v25。`--no-point-cloud` 可关闭即时推导。
...
录制 schema v25 保存 aligned raw depth、RGB、native depth/color 几何 provenance、帧号和时间
信息，并持久化与 camera source 对齐的 arm/hand policy observation。离线处理只接受 raw v25；
...
| raw v25 相机 metadata 与点云输入适配 | `dataset/pointcloud.py` |
| raw v25 → processed v14 | `dataset/processing.py` |
