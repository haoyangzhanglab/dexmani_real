...
来源/顺序、policy ID、canonical table plane、sampling 和 transform；使用 fingertip 时，同一边界核对
FK derivation 与 algorithm policy ID。
...
处理数据记录的 derivation 和 algorithm ID 一致。
...
这些 summary 是 log-only experiment diagnostics：不落盘、不生成 receipt、sidecar 或部署资格证明。
SDK acceptance 只表示 worker/SDK 接受目标，不表示物理到位；它保留给 home、
