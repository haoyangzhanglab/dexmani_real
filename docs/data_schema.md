...
本文覆盖 raw HDF5 v29、processed HDF5 v20 与 Policy Zarr v13。精确合同由
...
`EpisodeReader` 只读当前 v29，校验必需文件、shape/dtype 和 sidecar 帧数；
...
| hand_contact | (5,3) | float32 |
| hand_contact_valid | scalar | bool |
| hand_tactile_force | (5,120,3) | float32 |
| hand_tactile_force_valid | scalar | bool |
...
`hand_contact` 是软件 bias 校正的 SDK `calc_force`（aggregate）。
`hand_tactile_force` 是 SDK `raw_force`（dense），昂贵的原始 dense 数据保留在 raw。
录制在每个 control anchor 选择最新 causal 的 XHand hand sample：qpos/current 与
aggregate/dense 来自同一次 SDK read，共享唯一 `hand_source_monotonic_ns`，不再有独立的
aggregate/dense source timestamp。`hand_contact_valid` / `hand_tactile_force_valid`
仅表示该 sample 的两个 bias-corrected 表示是否可用；它们不编码 freshness、contact
状态、单位或 magnitude。无有效 payload 时保存 NaN + valid=False，不把无效读数伪装成
无接触零值。单位为 `xhand_sdk_native_unknown_si`，SI Newton 与 taxel 空间几何均未验证；
...
历史 raw 保持历史版本。冻结 v24→v25 与 v25→v26 工具保留；不提供 v26/v27→v29
伪迁移，也不原地改写原始文件。直接 v25 的旧 0.1 tactile scale 由冻结转换器还原；
v24-derived 数据必须先证明 lineage，再显式选择 scale，不能靠数值大小猜测。
旧 raw 若需 reprocess，应使用拥有其语义的历史仓库版本；当前 runtime reader、raw
viewer、physical replay 与 offline processing 都没有历史版本兼容路径。实际
pick_place_toy lineage、不可恢复的信息限制及验证结果见
[incident evidence](invalid_frames_export_incident.md) 和
[salvage manifest](../artifacts/pick_place_toy_salvage_manifest.json)。

## 2. Processed HDF5 v20
...
| contact_force_valid | (N,) | bool | 直接复制 hand_contact_valid |
| tactile_force | (N,5,120,3) | float32 | hand_tactile_force |
| tactile_force_valid | (N,) | bool | 直接复制 hand_tactile_force_valid |
...
无效 contact/dense 用 validity mask 表达（`*_valid=False` 时 payload 为 NaN），不整条拒绝。
tactile validity 是 raw 的直接拷贝，processing 不重建 measurement truth：它不读取或推断
calibration、unit、freshness 或独立 source timestamp。zero payload 不天然 invalid——mask
只来自所选 causal hand sample 的 `tactile_aggregate_valid` / `tactile_dense_valid`
（worker 已把 SDK status、parse/finite 与 session software-bias 合入这两个 bit）。
aggregate 与 dense 的可用性保持相互独立。
processed 保存 `eef_pose`（v20 起）：`eef_pose` 与本行 `fingertip_points` 复用同一次
...
| 属性 | 值/含义 |
|---|---|
| schema_name / schema_version | dexmani-real-processed-hdf5 / 20 |
...
qpos_stale、camera health 全部保留；contact_force/tactile_force 直接读取同一 hand_state
history 中的 `tactile_aggregate`/`tactile_dense` 并分别要求 `tactile_aggregate_valid`/
`tactile_dense_valid`，不再有独立的 provenance ring、source-match 或 calibration/unit
gate。训练接纳 sensor imperfection 不代表在线放宽准入。command safety、collision、
generation、freshness、shutdown 不变。
