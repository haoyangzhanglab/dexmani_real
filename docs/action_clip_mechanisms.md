...
2. learned-policy arm 是 reject-only：wrap-aware 最近等价表示 + 绝对限位准入后，超 joint
   limit 或 per-joint jump 阈值直接拒绝，绝不 clip；hand 同样是 reject-only。首条动作相对
   measured feedback，后续动作相对上一条已接受的 logical target。
...
|---|---|---|---|---|---|
| VR `VRWristMapper` | wrist rotation | 限制相邻接受姿态的单帧旋转增量 | 构造器默认 `max_per_frame_rot_rad=0.52 rad`，约 `29.8°`；当前调用链没有单独 runtime override | 以 mapper 上一帧接受的 wrist rotation 为基准；超限时压回最大允许旋转 | [`vr_mapping.py:138`](../dexmani_real/teleop/control_loop/vr_mapping.py#L138)、[`loop.py:153`](../dexmani_real/teleop/loop.py#L153) |
| VR `VRWristMapper` | 从 reset anchor 累积的 wrist rotation | 限制相对 reset 姿态的累计旋转 | `runtime.policy.vr_mapping.max_delta_rot_rad=3.0 rad`，约 `171.9°` | 以 reset 时的参考姿态为基准；达到边界后压回。若需要继续转动，必须重新建立 reset/re-anchor | [`vr_mapping.py:222`](../dexmani_real/teleop/control_loop/vr_mapping.py#L222)、[`defaults.py:119`](../dexmani_real/config/defaults.py#L119) |
| VR EEF target proposal | EEF position | 工作空间 `np.clip` | 默认 `x=[0.25,0.72] m`、`y=[-0.50,0.50] m`、`z=[0.05,0.50] m` | 在 EMA 之后对 world/arm-base 目标位置限幅；限幅前位置另存为 `target_pos_before_clamp` | [`defaults.py:62`](../dexmani_real/config/defaults.py#L62)、[`action_proposal.py:77`](../dexmani_real/teleop/control_loop/action_proposal.py#L77)、[`action_proposal.py:109`](../dexmani_real/teleop/control_loop/action_proposal.py#L109)、[`grid.py:774`](../dexmani_real/teleop/control_loop/grid.py#L774) |
| VR arm action proposal | 7 个 arm joint | 先做 joint bound clip，再做相对上一条 arm command 的 wrap-aware delta clip，最后再做 joint bound clip | `teleop_arm_max_delta_rad_per_tick=8°=0.139626 rad` | 相对上一条已发布/接受的 arm command；角度按最近等价表示比较；这是 producer-side 修改 | [`action_proposal.py:239`](../dexmani_real/teleop/control_loop/action_proposal.py#L239)、[`defaults.py:642`](../dexmani_real/config/defaults.py#L642) |
| learned-policy arm | 7 个 arm joint | 限位内最近等价表示 → 绝对限位准入 → per-joint jump reject | `policy.arm_max_action_jump_rad=20°=0.349066 rad` | 首条相对 measured feedback，后续相对上一条 accepted logical target；joint 与 EE→IK 共用此顺序，超阈值直接拒绝（不 clip），workspace 检查实际 canonical 路径 | [`executor.py`](../dexmani_real/deployment/executor.py) |
| VR hand action proposal | 12 个 hand joint | 先压到 operational command bounds，再限制相对上一条 published hand endpoint 的 delta | `hand_max_delta_rad_per_tick=0.3 rad` | 以上一条 published endpoint 为参考；每个 joint 独立限幅 | [`action_proposal.py:123`](../dexmani_real/teleop/control_loop/action_proposal.py#L123)、[`action_proposal.py:236`](../dexmani_real/teleop/control_loop/action_proposal.py#L236)、[`defaults.py:499`](../dexmani_real/config/defaults.py#L499) |
| TAG retarget optimizer | 内部 `target_factors` | 对优化变量做 box constraint | `target_factors ∈ [0,1]` | 这是 retarget 内部变量边界，不是最终 arm/hand SDK command 的 clip | [`tag_optimizer.py:223`](../dexmani_real/teleop/retargeting/tag_optimizer.py#L223) |
| TAG retarget optimizer | optimizer joint result / warm start | 按 URDF joint bounds 压回可行范围 | 每个 joint 使用对应 URDF lower/upper bound | 作用于优化过程的候选 q；之后仍需经过上层 IK/安全路径 | [`tag_optimizer.py:270`](../dexmani_real/teleop/retargeting/tag_optimizer.py#L270) |
| Keyboard / camera calibration EEF command | EEF position | 工作空间 clip，并留出 command margin | `workspace_command_margin_m=0.005 m`；默认有效范围为 `x=[0.255,0.715]`、`y=[-0.495,0.495]`、`z=[0.055,0.495] m` | 这是相对 nominal workspace 向内收缩 `5 mm` 后的 command workspace | [`keyboard_session.py:539`](../dexmani_real/teleop/keyboard_session.py#L539)、[`motion.py:286`](../dexmani_real/calibration/camera/motion.py#L286) |
...
| `policy.arm_max_action_jump_rad` | `0.349066` | rad，等于 `20°` | learned-policy arm per-joint jump 的 reject 阈值；不属于 SafetyGate 或 worker |
...
| `OnlineIKConfig.nullspace_step_size_deg`（keyboard/calibration 默认） | `1` | °/step | teleop IK nullspace 单步调整量 |
...
|---|---|---|---|---|
| Teleop IK candidate | 相邻 IK q candidate 的 joint jump | `max_ik_jump_deg=(30,30,30,35,40,40,40)°` | reject candidate；不会把 q 改成边界值 | [`ik.py:49`](../dexmani_real/planning/kinematics/ik.py#L49)、[`ik.py:265`](../dexmani_real/planning/kinematics/ik.py#L265) |
| Teleop IK pose validation | EEF position/orientation error | VR 默认 `0.02 m`、`5°`；keyboard/calibration 默认 `0.002 m`、`2°` | reject candidate | [`defaults.py:637`](../dexmani_real/config/defaults.py#L637)、[`defaults.py:749`](../dexmani_real/config/defaults.py#L749)、[`ik.py:269`](../dexmani_real/planning/kinematics/ik.py#L269) |
...
| learned-policy `PolicyExecutor` | arm neural endpoint / hand endpoint delta | arm 使用独立 `policy.arm_max_action_jump_rad`；hand 使用 `policy.hand_max_action_jump_rad`；首条相对 measured、后续相对上一条 accepted logical target | arm 超限分量直接 reject（不 clip）；非法绝对限位或 hand jump 拒绝 coupled step，拒绝时 reference 不推进 | [`executor.py`](../dexmani_real/deployment/executor.py)、[`safety_gate.py`](../dexmani_real/control/safety_gate.py) |
...
字段定义和写入路径：[`storage/schema.py:47`](../dexmani_real/recording/storage/schema.py#L47)、[`storage/schema.py:96`](../dexmani_real/recording/storage/schema.py#L96)、[`storage/schema.py:103`](../dexmani_real/recording/storage/schema.py#L103)、[`recording/client.py:175`](../dexmani_real/recording/client.py#L175)、[`grid.py:711`](../dexmani_real/teleop/control_loop/grid.py#L711)。
...
Learned policy:
  model Prediction → PolicyExecutor decode
                   → arm wrap/absolute-limit/jump reject-only
